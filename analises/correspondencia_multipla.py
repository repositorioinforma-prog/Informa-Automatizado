...
Permite escolher uma variável principal e cruzar com várias variáveis
secundárias de uma vez, gerando um mapa editável para cada par, com
exportação de todos os mapas para PowerPoint ao final.

Não depende de `analises.correspondencia_simples` e não a modifica.
"""
import io
...
def _preparar_estado_linhas_principal(dados, variavel_principal):
    ...
    Na primeira vez que a variável é configurada (ainda não há nada em
    session_state para ela), se as categorias baterem com a escala de
    intenção de voto (CATEGORIAS_ESCALA_INTENCAO_VOTO), só as categorias
    de CATEGORIAS_MANTER_PADRAO_INTENCAO_VOTO já vêm marcadas para exibir;
    as demais já entram marcadas para remover. Edições feitas depois pelo
    usuário são sempre respeitadas (este pré-preenchimento só roda uma vez).
    """
    ...
    if key_rotulos not in st.session_state:
        categorias_normalizadas = {_normalizar_categoria(c) for c in categorias}
        eh_escala_intencao_voto = (
            categorias_normalizadas
            and categorias_normalizadas <= CATEGORIAS_ESCALA_INTENCAO_VOTO
            and categorias_normalizadas & CATEGORIAS_MANTER_PADRAO_INTENCAO_VOTO
        )

        st.session_state[key_rotulos] = {}
        for c in categorias:
            if eh_escala_intencao_voto and (
                _normalizar_categoria(c) not in CATEGORIAS_MANTER_PADRAO_INTENCAO_VOTO
            ):
                st.session_state[key_rotulos][c] = None  # já vem pré-marcada para remover
            else:
                st.session_state[key_rotulos][c] = str(c)
    else:
        for c in categorias:
...
def _gerar_pptx_mapas(figuras):
    ...
    for titulo, fig in figuras:
        ...
        # remove o título interno do matplotlib ("Variável Principal x
        # Variável Cruzada") antes de exportar a imagem — o slide já tem
        # seu próprio título em texto (caixa_titulo, acima). Isso só afeta
        # a imagem exportada: a tela já foi renderizada antes deste ponto,
        # então o mapa exibido no app continua mostrando o título normalmente.
        for ax in fig.axes:
            ax.set_title("")
